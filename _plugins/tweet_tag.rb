...
module Jekyll
  class TweetTag < Liquid::Tag
    ...
    def cached_response(api_params)
      ...
      cache_file = cache_file_for(api_params)
      cached_item = File.read(cache_file) if File.exist?(cache_file)
      parsed = cached_item && cached_item.length >= 2 ? JSON.parse(cached_item) : nil
      parsed
    end
    ...
    def live_response(api_params)
      ...
      parsed = response && response.length >= 2 ? JSON.parse(response) : nil
      parsed
    end
  end
